...
@app.get("/")
async def root():
    return {"status": "online", "message": "Imperium Challenge API v1.0.0"}
...
def login(data: LoginRequest):
    """Login participant (supports Email or Username)."""
    search_term = data.email.lower()
    for user_id, user in users_db.items():
        # Check if input matches either email OR name (case-insensitive)
        if (user["email"].lower() == search_term or user["name"].lower() == search_term) and user["password"] == data.password:
            return {"user_id": user_id, "message": "Logged in successfully"}
    
    raise HTTPException(status_code=401, detail="Invalid credentials. Try email: alice@example.com or name: Alice Hacker")
