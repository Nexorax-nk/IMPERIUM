...
@keyframes imp-rpl{from{width:0;height:0;opacity:.9;transform:translate(-50%,-50%)}to{width:130px;height:130px;opacity:0;transform:translate(-50%,-50%)}}`;
    ...
    const S = { mx:0,my:0,ox:0,oy:0,ix:0,iy:0,vx:0,vy:0,mode:"default",modeProgress:0,sparks:[] as any[],ringAngle:0,radarAngle:0,magnetX:0,magnetY:0,magnetTarget:null as any };

    const resize = () => { canvas.width = window.innerWidth; canvas.height = window.innerHeight; };
    resize();
    window.addEventListener("resize", resize, { passive: true });

    function detectMode(el: Element | null) {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const labels: Record<string, string> = { default:"SYS.TRACK", button:"ACTIVATE", link:"NAVIGATE", card:"SCAN", text:"READ" };
    ...
    function frame(ts: number) {
      ...
      const radiusMap: Record<string, number> = { default:26, button:38, link:20, card:44, text:18 };
      const coreMap: Record<string, number> = { default:5, button:7, link:3, card:8, text:2.5 };
      const outerR = radiusMap[S.mode] ?? 26;
      const coreR  = coreMap[S.mode] ?? 5;
          ...
          {showCta && (
            <div style={{marginTop:36,display:"flex",flexDirection:"column",alignItems:"center",gap:12,animation:"fadeIn .6s ease"}}>
              ...
              <button onClick={onEnter} style={{fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:700,letterSpacing:4,padding:"14px 48px",background:"transparent",color:"#ff3b5c",border:"1.5px solid rgba(255,59,92,.6)",cursor:"pointer",clipPath:"polygon(12px 0%,100% 0%,calc(100% - 12px) 100%,0% 100%)",transition:"all .2s",textTransform:"uppercase",boxShadow:"0 0 20px rgba(255,59,92,.2)"}}
                onMouseEnter={e=>{(e.target as any).style.background="rgba(255,59,92,.1)";(e.target as any).style.boxShadow="0 0 40px rgba(255,59,92,.5)";}}
                onMouseLeave={e=>{(e.target as any).style.background="transparent";(e.target as any).style.boxShadow="0 0 20px rgba(255,59,92,.2)";}}>
